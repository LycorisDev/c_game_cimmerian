...
{
    MESH_VIEWPORT = create_mesh(SHAPE_VIEWPORT);
...
static void create_mesh_buffer_objects(Mesh* mesh)
{
    ...
    */
    for (i = 0; i < mesh->nbr_attributes; ++i)
    {
        ...
        glVertexAttribPointer(i, mesh->attr_len, GL_FLOAT, GL_FALSE, 
            mesh->attr_len * mesh->nbr_attributes * sizeof(GLfloat), 
            (const void*)(mesh->attr_len * i * sizeof(GLfloat)));
    }
