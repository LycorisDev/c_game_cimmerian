...
static int	g_movement_input[3] = {0};
static int	g_rotation_input = 0;
...
static void	update_movement_action(int axis)
{
	...
	input = g_movement_input[axis];
	if (input < 0)
		g_man.movement_action[axis] = -1;
	else if (input > 0)
		g_man.movement_action[axis] = 1;
	else
		g_man.movement_action[axis] = 0;
	return ;
}

static void	update_rotation_action(void)
{
	if (g_rotation_input < 0)
		g_man.rotation_action = -1;
	else if (g_rotation_input > 0)
		g_man.rotation_action = 1;
	else
		g_man.rotation_action = 0;
	return ;
}
...
static void	input_escape(GLFWwindow *window, int action)
{
	/*
		...
		`glfwDestroyWindow(window)`, or the whole program (windows included) 
		can be closed with `glfwTerminate()`.
	*/
