...
void	init_input_handling(t_man *man)
{
	...
	sua_hook(man->xvar, ButtonPress, mouse_press_callback, man);
	sua_hook(man->xvar, ButtonRelease, mouse_release_callback, man);
	sua_hook(man->xvar, MotionNotify, mouse_move_callback, man);
	sua_hook(man->xvar, FocusOut, cancel_keys_in_game_state, man);
	sua_hook(man->xvar, ConfigureNotify, move_or_resize_window_callback, man);
	sua_loop_hook(man->xvar, game_loop, man);
	return ;
}
