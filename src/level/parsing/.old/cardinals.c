...
int n_texture)
{
	...
	while (*line)
	{
		if ((!man->bonus && args >= 6) || (!strncmp(*line, "D ", 2)
				&& check_texfilled(map->types, &n_texture)))
			return ;
		else if (is_bad_map(line) || n_texture < 0)
			break ;
		else if (find_n_wall(*line, &n_wall, &type))
		{
			if (n_wall > 8)
...
static int	is_bad_map(char **line)
{
	...
	if (!onlyvalids(*line, WHITES))
	{
		...
		if (strlen(*line) < 7)
			return (1);
		if (!strncmp(*line, "F", 1) || !strncmp(*line, "C", 1)
			|| !strncmp(*line + digit, "SO", 2)
			|| !strncmp(*line + digit, "WE", 2)
			|| !strncmp(*line + digit, "EA", 2)
			|| !strncmp(*line + digit, "NO", 2))
			return (0);
		else
			return (1);
	}
