#include "cimmerian.h"

static void	init_ray_data_x(t_ray *r, double cam_x);
static void	init_ray_data_y(t_ray *r, double cam_x);
static void	update_ray_data(t_ray *r);

void	perform_dda(t_map *m, double cam_x, t_list **list)
{
	t_ray	*r;
	double	biggest_height;
	int		add;

	r = calloc(1, sizeof(t_ray));
	if (!r)
		return ;
	init_ray_data_x(r, cam_x);
	init_ray_data_y(r, cam_x);
	biggest_height = 0;
	while (1)
	{
		update_ray_data(r);
		add = dda_add_to_list(m, r, &biggest_height);
		if (add < 0)
			break ;
		else if (add)
		{
			list_add_front(list, list_new(r));
			r = calloc(1, sizeof(t_ray));
			memcpy(r, ((t_ray *)(*list)->data), sizeof(t_ray));
		}
	}
	free(r);
	return ;
}

static void	init_ray_data_x(t_ray *r, double cam_x)
{
	r->ray_dir.x = g_man.player.dir.x + g_man.player.plane.x * cam_x;
	r->delta_dist.x = INF;
	if (r->ray_dir.x)
		r->delta_dist.x = abs_f(1 / r->ray_dir.x);
	r->m_index.x = (int)g_man.player.pos.x;
	if (r->ray_dir.x < 0)
	{
		r->step.x = -1;
		r->side_dist.x = (g_man.player.pos.x - r->m_index.x) * r->delta_dist.x;
	}
	else
	{
		r->step.x = 1;
		r->side_dist.x = (r->m_index.x + 1.0 - g_man.player.pos.x)
			* r->delta_dist.x;
	}
	return ;
}

static void	init_ray_data_y(t_ray *r, double cam_x)
{
	r->ray_dir.y = g_man.player.dir.y + g_man.player.plane.y * cam_x;
	r->delta_dist.y = INF;
	if (r->ray_dir.y)
		r->delta_dist.y = abs_f(1 / r->ray_dir.y);
	r->m_index.y = (int)g_man.player.pos.y;
	if (r->ray_dir.y < 0)
	{
		r->step.y = -1;
		r->side_dist.y = (g_man.player.pos.y - r->m_index.y) * r->delta_dist.y;
	}
	else
	{
		r->step.y = 1;
		r->side_dist.y = (r->m_index.y + 1.0 - g_man.player.pos.y)
			* r->delta_dist.y;
	}
	return ;
}

static void	update_ray_data(t_ray *r)
{
	if (r->side_dist.x < r->side_dist.y)
	{
		r->side_dist.x += r->delta_dist.x;
		r->m_index.x += r->step.x;
		r->side = 0;
	}
	else
	{
		r->side_dist.y += r->delta_dist.y;
		r->m_index.y += r->step.y;
		r->side = 1;
	}
	return ;
}
